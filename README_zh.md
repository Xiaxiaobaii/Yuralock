...
3. 在"输入密钥"输入框输入你要使用的密钥（或者不输入任何密钥
...
6. 等待一段时间，或者将加密文件分享给别人，或者什么都不做，打开App,选择加密后的文件，输入在第三步设置的密钥（如果没有设置密钥，直接点击解密即可），点击"开始解密"，等待解密完成后会有弹窗告诉您解密后的文件存储在哪个位置（一般是存储在与加密文件相同的位置，并且文件名与加密时选择文件的文件名相同）
